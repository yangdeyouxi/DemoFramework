...
import demoframework.yangmo.com.injectionlibrary.viewinjection.annotation.OnClickEvent;
...
public class ViewInjectUtils {
    ...
    private static void injectOnClickListener(Activity activity){
        Class<? extends Activity> clazz = activity.getClass();
        Method[] methods= clazz.getDeclaredMethods();//获取所有声明为公有的方法
        ...
        for (Method method:methods){//遍历所有公有方法
            ...
            for (Annotation annotation:annotations){//遍历所有注解
                ...
                OnClickEvent onClickEvent = annotationType.getAnnotation(OnClickEvent.class);//取出注解的onClickEvent注解，在定义中，只有InjectOnClick有注解为OnClickEvent
                if(onClickEvent!=null){//如果不为空
                    try {
                        Method valueMethod=annotationType.getDeclaredMethod("value");//获取注解InjectOnClick的value方法
                        int[] viewIds= (int[]) valueMethod.invoke(annotation);//获取控件值
